...
class GmeQwen2VL:
    ...
    def get_embeddings(self, texts: list[str] = None, images: list[Image.Image] | DataLoader = None,instruction = None, **kwargs):
        if images is None:
            embeddings = self.embed(texts=texts,images=[None]*len(texts),instruction=instruction, **kwargs)
        else:
            embeddings = self.embed(texts=texts, images=images,instruction=instruction, **kwargs)
        return embeddings
